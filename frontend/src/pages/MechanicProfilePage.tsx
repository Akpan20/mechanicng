...
function Stars({ rating }: { rating: number }) {
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map(i => (
        <span key={i} className={`text-sm ${i <= Math.round(rating) ? 'text-amber-400' : 'text-gray-700'}`}>★</span>
      ))}
    </div>
  )
}
...
export default function MechanicProfilePage() {
  ...
  const onQuoteSubmit = async (data: QuoteForm) => {
    try {
      ...
      toast.error((err as Error).message ?? 'Failed to send quote')
    }
  }
        ...
        <div className="card overflow-hidden mb-4">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <span className="text-sm text-gray-400">
                    {mechanic.rating > 0
                      ? `${mechanic.rating} · ${reviewCount} review${reviewCount !== 1 ? 's' : ''}`
                      : 'No reviews yet'}
                  </span>
                </div>
              </div>
          ...
          <div className="p-6 grid grid-cols-2 sm:grid-cols-4 gap-3 border-b border-gray-800">
            ...
            {hasLocation ? (
              ...
              <button disabled
                className="btn-outline flex items-center justify-center gap-2 text-sm py-3 opacity-40 cursor-not-allowed">
                🗺️ Directions
              </button>
            )}
          ...
          <div className="p-6 grid grid-cols-2 gap-4 border-b border-gray-800">
            {([
              ...
              ['📍 Type',    mechanic.type === 'mobile'
                ? `Mobile${mechanic.service_radius ? ` · ${mechanic.service_radius}km radius` : ''}`
                : 'Auto Shop'],
          ...
          {mechanic.bio && (
            <div className="p-6 border-b border-gray-800">
              ...
              <p className="text-gray-300 leading-relaxed"
                dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(mechanic.bio) }} />
            </div>
          )}
          ...
          <div className="p-6">
            ...
            <div className="flex flex-wrap gap-2">
              {mechanic.services.map(s => (
                <span key={s} className="tag text-sm py-1.5 px-3">{s}</span>
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="card overflow-hidden mb-4">
          <div className="p-6 border-b border-gray-800 flex items-center justify-between">
            ...
            {!user && (
              <span className="text-xs text-gray-500">
                <Link to="/login" className="text-brand-500 hover:underline">Sign in</Link> to leave a review
              </span>
            )}
          </div>
            ...
            <div className="p-6 border-b border-gray-800 bg-gray-900/50">
              ...
              <form onSubmit={handleReviewSubmit(onReviewSubmit)} className="space-y-4">
                <div>
                  ...
                  <StarPicker
                    value={ratingValue}
                    onChange={v => setReviewValue('rating', v, { shouldValidate: true })}
                  />
                  {reviewErrors.rating && (
                ...
                <div>
                  ...
                  {reviewErrors.comment && (
                    <p className="text-red-400 text-xs mt-1">{reviewErrors.comment.message}</p>
                  )}
                </div>
          ...
          {totalPages > 1 && (
            <div className="p-4 border-t border-gray-800 flex items-center justify-center gap-3">
              <button className="btn text-sm" onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page <= 1}>
                Previous
              </button>
              <span className="text-sm text-gray-400">Page {page} of {totalPages}</span>
              <button className="btn text-sm" onClick={() => setPage(p => p + 1)} disabled={page >= totalPages}>
                Next
              </button>
            </div>
          )}
              ...
              <form onSubmit={handleSubmit(onQuoteSubmit)} className="space-y-4">
                ...
                <div>
                  ...
                  <textarea className="input resize-none" rows={3}
                    {...register('note')} placeholder="Describe the issue..." />
                </div>
                <div className="flex gap-3 pt-2">
                  <button type="button" onClick={() => setShowQuote(false)} className="btn-outline flex-1">
                    Cancel
                  </button>
