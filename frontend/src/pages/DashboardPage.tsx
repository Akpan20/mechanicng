...
import { PLAN_COLORS, PLANS } from '@/lib/constants'
              ...
              <div className="card p-5 border-amber-500/30 bg-amber-500/5">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-400">Your listing is being reviewed by our team. This usually takes 24–48 hours. You'll receive an email when it's approved.</p>
                  </div>
                </div>
              </div>
            )}
        ...
        {activeTab === 'listing' && (
          <div className="card p-7 space-y-5">
            <div className="flex justify-between items-center">
              ...
              <button onClick={() => setEditMode(!editMode)} className="btn-outline text-sm py-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              {([
                ...
                ['Area',          mechanic.area ?? '',  'area'     ],
                ['Hours',         mechanic.hours,       'hours'    ],
            ...
            <div>
              <p className="section-title mb-2">Services</p>
            ...
            {editMode && (
              <button onClick={() => {
                updateMechanic.mutate({ id: mechanic.id, data: editForm })
                setEditMode(false)
                    ...
                    <div className="flex justify-between items-start flex-wrap gap-3">
                      ...
                      {q.status === 'pending' && (
                        <div className="flex gap-2 flex-wrap">
                          <a href={`tel:${q.customer_phone}`} className="btn-primary text-sm py-2 px-3">📞 Call</a>
                          ...
                          <button onClick={() => handleMarkResponded(q.id)}
        ...
        {activeTab === 'subscription' && (
          <div className="space-y-5">
            <div className="card p-7 text-center" style={{ borderColor: planColor + '40' }}>
              ...
              <h2 className="text-3xl font-extrabold mb-1" style={{ color: planColor }}>{currentPlan.name} Plan</h2>
              <p className="text-gray-400 mb-2">
                {currentPlan.priceNGN === 0 ? 'Free forever' : `₦${currentPlan.priceNGN.toLocaleString()}/month`}
              </p>
              {mechanic.plan !== 'free' && (
                  ...
                  <div key={plan.id} className={`card p-5 ${plan.highlighted ? 'border-brand-500/40' : ''}`}>
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        ...
                        <p className="text-brand-500 font-extrabold font-mono">₦{plan.priceNGN.toLocaleString()}/mo</p>
                      </div>
                      {plan.badge && <span className="badge text-xs bg-brand-500/20 text-brand-500">{plan.badge}</span>}
                    </div>
