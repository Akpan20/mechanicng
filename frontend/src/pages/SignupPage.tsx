...
export default function SignupPage() {
  ...
  const onAccountSubmit = async (data: AccountForm) => {
    if (role === 'user') {
      // Regular users: create account and done
      setLoading(true)
      try {
      ...
      // Mechanics: proceed to listing step
      setAccountData(data)
      ...
      <div className="max-w-xl mx-auto px-4 py-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <button onClick={() => setRole('user')}
            ...
            <h2 className="text-lg font-extrabold mb-1 group-hover:text-brand-500 transition-colors">I need a mechanic</h2>
            <p className="text-sm text-gray-400">Find trusted mechanics, request quotes, and leave reviews.</p>
          </button>
          <button onClick={() => setRole('mechanic')}
            className="card p-7 text-left hover:border-brand-500/50 transition-all group">
            <div className="text-4xl mb-3">🔧</div>
            <h2 className="text-lg font-extrabold mb-1 group-hover:text-brand-500 transition-colors">I'm a mechanic</h2>
            <p className="text-sm text-gray-400">List your business and get discovered by thousands of drivers.</p>
          </button>
        </div>
  ...
  return (
    <>
      <Helmet><title>{role === 'mechanic' ? 'List Your Business' : 'Sign Up'} – MechanicNG</title></Helmet>
      <div className="max-w-xl mx-auto px-4 py-10">
        <div className="text-center mb-8">
          <p className="section-title mb-2">Join MechanicNG</p>
          ...
          <button onClick={() => setRole(null)} className="text-xs text-gray-500 hover:text-gray-300 mt-2 underline">
            ← Change account type
          </button>
        </div>

        {/* Step indicator — only for mechanics (3 steps) */}
        {role === 'mechanic' && (
          <div className="flex items-center mb-8">
            {STEPS.map((label, i) => (
              <div key={i} className="flex-1 flex items-center">
                <div className={`flex items-center gap-2 flex-1 ${i < STEPS.length - 1 ? 'after:flex-1 after:h-px after:mx-2 after:bg-gray-800' : ''} ${step > i + 1 ? 'after:bg-brand-500/40' : ''}`}>
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0 transition-all ${step > i + 1 ? 'bg-brand-500 text-white' : step === i + 1 ? 'bg-brand-500 text-white shadow-glow-brand' : 'bg-gray-800 text-gray-500'}`}>
                    {step > i + 1 ? '✓' : i + 1}
                  </div>
                  <span className={`text-xs font-semibold ${step === i + 1 ? 'text-white' : 'text-gray-500'}`}>{label}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <form onSubmit={accountForm.handleSubmit(onAccountSubmit)} className="card p-7 space-y-5">
            ...
            {([
              ...
              <div key={f.name}>
                ...
                <input className="input" type={f.type} placeholder={f.placeholder} {...accountForm.register(f.name)} />
                {accountForm.formState.errors[f.name] && (
                  <p className="text-red-400 text-xs mt-1">{accountForm.formState.errors[f.name]?.message}</p>
                )}
              </div>
            ))}
        ...
        {step === 2 && role === 'mechanic' && (
          <form onSubmit={listingForm.handleSubmit(onListingSubmit)} className="card p-7 space-y-5">
            ...
            <div>
              ...
              <input className="input" placeholder="e.g. Emeka AutoFix" {...listingForm.register('name')} />
              {listingForm.formState.errors.name && <p className="text-red-400 text-xs mt-1">{listingForm.formState.errors.name.message}</p>}
            </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {(['shop', 'mobile'] as const).map(t => (
                  <button key={t} type="button" onClick={() => listingForm.setValue('type', t)}
                    className={`p-4 rounded-xl border-2 font-bold text-base transition-all ${listingForm.watch('type') === t ? 'border-brand-500 bg-brand-500/10 text-brand-500' : 'border-gray-700 text-gray-400 hover:border-gray-500'}`}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input className="input" placeholder="0801 234 5678" {...listingForm.register('phone')}
                  onChange={e => {
            ...
            <div>
              ...
              {!samePhone && <input className="input" placeholder="WhatsApp number" {...listingForm.register('whatsapp')} />}
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                {listingForm.formState.errors.city && <p className="text-red-400 text-xs mt-1">{listingForm.formState.errors.city.message}</p>}
              </div>
              <div>
                <label className="section-title block mb-2">Area</label>
                <input className="input" placeholder="e.g. Victoria Island" {...listingForm.register('area')} />
              </div>
            </div>
            ...
            {listingForm.watch('type') === 'shop' && (
              <div>
                ...
                <input className="input" placeholder="Full address of your shop" {...listingForm.register('address')} />
              </div>
            )}

            {listingForm.watch('type') === 'mobile' && (
              <div>
                <label className="section-title block mb-2">Service Radius: {listingForm.watch('service_radius') ?? 15}km</label>
                ...
                <div className="flex justify-between text-xs text-gray-600 mt-1"><span>5km</span><span>100km</span></div>
              </div>
            )}
              ...
              <div className="flex flex-wrap gap-2">
                {SERVICES.map(s => (
                  <button key={s} type="button" onClick={() => toggleService(s)}
                    className={`px-3 py-2 rounded-lg border text-xs font-semibold transition-all ${selectedServices.includes(s) ? 'border-brand-500 bg-brand-500/10 text-brand-500' : 'border-gray-700 text-gray-400 hover:border-gray-500'}`}>
                    {selectedServices.includes(s) ? '✓ ' : ''}{s}
                  </button>
                ))}
              </div>
              {listingForm.formState.errors.services && <p className="text-red-400 text-xs mt-1">Select at least one service</p>}
            </div>

            <div>
              <label className="section-title block mb-2">Operating Hours *</label>
              <input className="input" placeholder="e.g. Mon-Sat 8am-6pm" {...listingForm.register('hours')} />
            </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                {(['low', 'mid', 'high'] as const).map(p => (
                  <button key={p} type="button" onClick={() => listingForm.setValue('price_range', p)}
                    className={`p-3 rounded-xl border-2 font-bold text-sm transition-all ${listingForm.watch('price_range') === p ? 'border-brand-500 bg-brand-500/10 text-brand-500' : 'border-gray-700 text-gray-400'}`}>
        ...
        {step === 3 && (
          <div className="card p-8 text-center">
            <div className="w-20 h-20 rounded-full bg-emerald-500/20 flex items-center justify-center mx-auto mb-6 text-4xl">✅</div>
            ...
            <div className="space-y-3">
              {role === 'mechanic' ? (
                <>
                  ...
                  <button onClick={() => navigate('/pricing')}   className="btn-outline w-full">View Subscription Plans</button>
                </>
              ) : (
                <button onClick={() => navigate('/')} className="btn-primary w-full">Find a Mechanic</button>
              )}
            </div>
          </div>
        )}
