    ...
    <Link to={`/mechanic/${m.id}`} className={`card block p-5 cursor-pointer group ${className}`}>
      ...
      <div className="flex gap-3.5 items-start mb-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 mb-0.5">
            <h3 className="font-bold text-gray-100 truncate group-hover:text-brand-500 transition-colors">{m.name}</h3>
            {m.verified && <span className="text-emerald-400 text-sm">✓</span>}
          </div>
          <p className="text-gray-500 text-sm">
            📍 {m.area && `${m.area}, `}{m.city}
            {m.distance !== undefined && <span className="ml-1 text-brand-500/80">{formatDistance(m.distance)}</span>}
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 mb-3">
        <div className="flex gap-0.5">
          {[1,2,3,4,5].map(i => (
            <span key={i} className={`text-sm ${i <= Math.round(m.rating) ? 'text-amber-400' : 'text-gray-700'}`}>★</span>
          ))}
        </div>
        <span className="text-sm text-gray-400">
          {m.rating > 0 ? `${m.rating} (${m.review_count})` : 'No reviews yet'}
        </span>
      </div>

      <div className="flex flex-wrap gap-1.5 mb-3">
        {m.services.slice(0, 3).map(s => <span key={s} className="tag">{s}</span>)}
        {m.services.length > 3 && <span className="tag">+{m.services.length - 3}</span>}
      </div>

      <div className="flex items-center justify-between text-xs">
        <div className="flex items-center gap-2">
          <span className={`font-semibold ${m.status === 'approved' ? 'text-emerald-400' : 'text-gray-500'}`}>
            🟢 Open
          </span>
          <span className="text-gray-600">·</span>
          <span className="text-gray-500">{m.type === 'mobile' ? '📱 Mobile' : '🏪 Shop'}</span>
        </div>
