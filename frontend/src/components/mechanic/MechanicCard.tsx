...
export default function MechanicCard({ mechanic: m, className = '', showAdminActions = false, onDelete }: Props) {
  ...
  return (
    <div className={`card block p-5 cursor-pointer group ${className}`}>
      {/* We'll wrap the card content in a div, but keep the link behavior */}
      <Link to={`/mechanic/${m.id}`} className="block">
        {/* Plan badge */}
        {m.plan !== 'free' && (
