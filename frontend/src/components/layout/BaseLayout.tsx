    ...
    <>
      ...
      <nav className="sticky top-0 z-50 bg-surface-900/95 backdrop-blur-sm border-b border-gray-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between h-14 sm:h-16">
          ...
          >
            <div
              className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-gradient-to-br from-brand-500 to-red-500
            ...
            {isAuthenticated ? (
              <div className="flex items-center gap-3">
                <span className="text-sm text-gray-400 truncate max-w-[140px]">
                  ...
                  {isAdmin && (
                    <span className="ml-2 badge bg-brand-500/20 text-brand-500 text-xs">
                      Admin
                    </span>
                  )}
                </span>
        ...
        {/* Mobile drawer */}
        <div
          className={`md:hidden border-t border-gray-800 bg-surface-900 overflow-hidden transition-all duration-300 ease-in-out ${
              ...
              {isAuthenticated ? (
                <>
                  <div className="px-3 py-2 text-xs text-gray-500 truncate">
                    {profile?.full_name || profile?.email}
                    {isAdmin && (
                      <span className="ml-2 badge bg-brand-500/20 text-brand-500">
                        Admin
                      </span>
                    )}
                  </div>
      ...
      <footer className="border-t border-gray-800 bg-surface-900 pt-10 pb-6 px-4 sm:px-6">
        <div className="max-w-7xl mx-auto">
          {/* Grid: 1 column on mobile, 2 on sm, 5 on md+ */}
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-6 sm:gap-8 mb-8">
            {/* Brand (spans full width on mobile, then normal column) */}
            ...
            {/* For Drivers */}
            <div>
              <h4 className="font-bold text-gray-300 mb-3 text-xs uppercase tracking-wider">
                For Drivers
              </h4>
