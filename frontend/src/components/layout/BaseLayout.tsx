    ...
    <div className="min-h-screen flex flex-col">
      ...
      <nav className="sticky top-0 z-50 bg-surface-900/95 backdrop-blur-sm border-b border-gray-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between h-14 sm:h-16">
          ...
          {/* Logo */}
          <Link to="/" className="flex items-center gap-2 group flex-shrink-0" onClick={close}>
            <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-gradient-to-br from-brand-500 to-red-500
              flex items-center justify-center text-lg sm:text-xl shadow-glow-brand group-hover:scale-105 transition-transform">
              🔧
            </div>
            ...
            {isAuthenticated ? (
              <div className="flex items-center gap-3">
                <span className="text-sm text-gray-400 truncate max-w-[140px]">
                  ...
                  {isAdmin && <span className="ml-2 badge bg-brand-500/20 text-brand-500 text-xs">Admin</span>}
                </span>
              ...
              {isAuthenticated ? (
                <>
                  <div className="px-3 py-2 text-xs text-gray-500 truncate">
                    {profile?.full_name || profile?.email}
                    {isAdmin && <span className="ml-2 badge bg-brand-500/20 text-brand-500">Admin</span>}
                  </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 sm:gap-8 mb-8">
            ...
            <div className="col-span-2 md:col-span-1">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="font-extrabold">Mechanic<span className="text-brand-500">NG</span></span>
              </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {([['/', 'Find a Mechanic'], ['/search', 'Search by City']] as const).map(([to, label]) => (
                  <Link key={to} to={to} className="block text-gray-500 hover:text-brand-500 text-sm transition-colors">{label}</Link>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {([['/signup', 'Create Listing'], ['/pricing', 'View Plans']] as const).map(([to, label]) => (
                  <Link key={to} to={to} className="block text-gray-500 hover:text-brand-500 text-sm transition-colors">{label}</Link>
                ))}
              </div>
            </div>
